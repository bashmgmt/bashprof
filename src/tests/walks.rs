...
async fn a_call_carries_the_whole_stack_it_was_made_on() {
    ...
    let c = calls(&recorded)
        .into_iter()
        .find(|call| call.label == "c")
        .expect("c was measured");

    assert_eq!(
        c.stack.top().site.to_string(),
        "f__B",
...
async fn a_wrapper_can_move_the_walk_past_itself() {
    ...
    assert_eq!(
        at(a, &["leaf"]).complete.call.stack.top().site.to_string(),
        "f__A",
        "the subject's site, not the wrapper's"
    );
    assert_eq!(
        a.complete.call.stack.top().site.to_string(),
        "main",
        "and the unwrapped call is unaffected\n{profile}"
    );
}
...
async fn a_span_says_where_its_call_was_made() {
    ...
    assert_eq!(
        a.complete.call.stack.top().site.to_string(),
        "main",
        "the outermost call is in the script's own body"
    );
    assert_eq!(
        at(a, &["b"]).complete.call.stack.top().site.to_string(),
        "f__A"
    );
    assert_eq!(
        at(a, &["e"]).complete.call.stack.top().site.to_string(),
        "f__A"
    );
    assert_eq!(
        at(a, &["b", "c"])
    ...
    assert_ne!(
        at(a, &["b", "c"]).complete.call.stack.top().lineno,
        at(a, &["b", "d"]).complete.call.stack.top().lineno
    );
}
