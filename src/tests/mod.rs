...
use crate::bash::rig::{heard, Driving, ExitStatus, Reached, Reaching};
...
async fn profiled(script: &str) -> (Vec<Recorded>, ExitStatus) {
    ...
    let ran = Reached { rig: BashProf, reaching: Reaching::BashEnv }.run(&bash(scripts.at("subject.bash"))).await.unwrap().whole().unwrap();
