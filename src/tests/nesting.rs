...
async fn measurements_nest_the_way_the_calls_do() {
    ...
    let labels = |span: &Span| {
        span.children
            .iter()
            .map(|c| c.complete.call.label.clone())
            .collect::<Vec<_>>()
    };
    ...
    assert!(
        a.all()
            .iter()
            .all(|span| span.complete.call.shell.pid == a.complete.call.shell.pid),
        "one shell produced all of it"
    );
}
...
async fn a_call_measured_in_a_subshell_nests_where_it_was_made() {
    ...
    assert_eq!(
        profile.roots.len(),
        1,
        "one outermost measurement, not one per shell"
    );
    let a = &profile.roots[0];
    ...
    assert_ne!(
        at(a, &["forked"]).complete.call.shell.pid,
        a.complete.call.shell.pid,
        "and it did run in a shell of its own"
    );
}
...
async fn concurrent_forks_of_one_line_keep_their_own_calls() {
    ...
    assert_eq!(
        a.children.len(),
        2,
        "one measurement per fork\n{profile}"
    );

    assert_ne!(
        a.children[0].complete.call.id, a.children[1].complete.call.id,
        "one line, two calls, two names"
    );
    ...
    for turn in &a.children {
        ...
        assert_eq!(
            turn.children.len(),
            1,
            "the call made in its own shell\n{profile}"
        );
        assert_eq!(
            turn.children[0].complete.call.shell.pid, turn.complete.call.shell.pid,
            "and no other's\n{profile}"
        );
    }
    ...
    assert!(
        together > a.complete.took(),
        "the two ran at once, so their windows overlap\n{profile}"
    );
    assert!(
        a.exclusive() < a.complete.took(),
        "and what neither covered is a's own\n{profile}"
    );
}
...
async fn a_name_is_inherited_through_two_levels_of_forking() {
    ...
    assert_eq!(
        labels,
        ["middle", "deep"],
        "both under a, neither under the other\n{profile}"
    );

    let pids = [
        a.complete.call.shell.pid,
        at(a, &["middle"]).complete.call.shell.pid,
        at(a, &["deep"]).complete.call.shell.pid,
    ];
    assert_eq!(
...
async fn every_measurement_has_a_name_of_its_own() {
    ...
    let names: HashSet<&str> = a
        .all()
        .iter()
        .map(|span| span.complete.call.id.0.as_str())
        .collect();
    assert_eq!(
        names.len(),
        6,
